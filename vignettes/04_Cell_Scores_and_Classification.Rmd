...
soleUMAP <- soleS %>%
  DimPlot(group.by = "integrated_snn_res.0.4") + NoLegend()
...
soleFeat <- soleS %>%
  FeaturePlot(features = names(sole.cell.scores), order = TRUE, min.cutoff = 0, 
              label = TRUE, repel = TRUE, ncol = 5)
...
## Solé et al. Figure 1.
```{}
(soleUMAP + soleHeat + plot_layouts(width = c(1, 2))) / soleFeat ) + 
  plot_layouts(guides = "collect")
```